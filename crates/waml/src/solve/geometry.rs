...
/// `sa`/`sb` are unused today but kept in the signature: it documents the
/// policy's inputs and both call sites already have them on hand.
        ...
        match c {
            Constraint::Place { a, b, dir } => {
                ...
                let gap = pair_gap(
                    ...
                    (sa, ma),
                    (sb, mb),
                    matches!(dir, Direction::LeftOf | Direction::RightOf),
                    connected,
                    label_widths,
                    cfg,
                );

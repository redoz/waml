...
/// note (spec §4). Test seam: the badge's error list is rendered by
/// `popup::conflict_list::rows_of`'s per-row `relation_statement`s; this is the
/// same copy in one-line summary form, for a tooltip/log that has not landed.
#[cfg(test)]
...
/// Test seam: the canvas fades by an explicit key set keyed off a single focused
/// RELATION's two nodes; the group-scoped ("All", spec §7) fade that wants every
/// participant has not landed.
#[cfg(test)]

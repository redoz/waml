...
script_mod! {
    ...
    mod.widgets.CompareProbe = set_type_default() do mod.widgets.CompareProbeBase{
        ...
        draw_svg: mod.draw.DrawSvg{ svg: crate_resource("self:resources/icons/square-plus.svg") }
    }
    ...
    startup() do #(App::script_component(vm)){
        ui: Root{
            main_window := Window{
                ...
                window.title: "square-plus: SVG vs SDF vs diff"
                body +: {
                    padding: 32
                    flow: Down
...
impl Widget for CompareProbe {
    fn draw_walk(&mut self, cx: &mut Cx2d, _scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        for &sz in SIZES.iter() {
            ...
            self.icons.square_plus.color = ACCENT;
            self.icons.square_plus.draw_abs(cx, Self::glyph(b, sz));
            ...
            self.icons.square_plus.color = OVL_SDF;
            self.icons.square_plus.draw_abs(cx, g);

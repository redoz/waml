...
script_mod! {
    ...
    mod.widgets.CompareProbe = set_type_default() do mod.widgets.CompareProbeBase{
        ...
        draw_svg: mod.draw.DrawSvg{ svg: crate_resource("self:resources/icons/pencil.svg") }
    }
    ...
    startup() do #(App::script_component(vm)){
        ui: Root{
            main_window := Window{
                ...
                window.title: "pencil: SVG vs SDF vs diff"
                body +: {
                    padding: 32
                    flow: Down
...
impl Widget for CompareProbe {
    fn draw_walk(&mut self, cx: &mut Cx2d, _scope: &mut Scope, walk: Walk) -> DrawStep {
        ...
        for &sz in SIZES.iter() {
            ...
            self.icons.pencil.color = ACCENT;
            self.icons.pencil.draw_abs(cx, Self::glyph(b, sz));
            ...
            self.icons.pencil.color = OVL_SDF;
            self.icons.pencil.draw_abs(cx, g);

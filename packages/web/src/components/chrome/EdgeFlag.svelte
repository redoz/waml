<script lang="ts">
  ...
  const base =
    "group absolute z-[12] hover:z-30 focus-visible:z-30 flex w-[34px] flex-col items-center justify-center gap-[6px] rounded-l-[var(--round-chip)] border border-r-0 border-[color:var(--hair)] bg-white py-3 text-[11px] font-[600] text-[color:rgb(var(--ink-faint))] shadow-[-3px_0_12px_rgba(40,70,110,.14)] cursor-pointer transition-[right,background-color,color] duration-200 hover:bg-[color:rgba(var(--accent),.10)] hover:text-[color:rgb(var(--accent))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(var(--accent),.6)]";
  const cls = $derived(base + (active ? " bg-[color:rgba(var(--accent),.12)] text-[color:rgb(var(--accent))]" : ""));

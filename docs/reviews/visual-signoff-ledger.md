...
| V18 | `?bundle=` boot no longer shows a blank editor | branch `a15-c4-web-boot` | human | A **one-line behaviour change** inside the C4 web-boot extraction, and the only user-visible part of it. The `?api=` boot response has always guarded `show_editor` on `open_bundle`'s return value — a bundle that fails session analysis must not present a blank editor. The `?bundle=` response ignored that return value, so a site whose `bundle.waml` fails analysis **showed an empty editor** over a session that had not been replaced; it now stays on the start screen with the error `open_bundle` already reported. (The same branch stops that failed open from leaving the export-time index-asset fetch armed, which would otherwise install a search index for the unopened bundle over whatever session is loaded — that half is covered by unit tests.) **Drive one case:** export a site (`waml export site <bundle> --out <dir>`), corrupt its `bundle.waml` so the bundle decodes but fails session analysis, serve the directory, and load it. The start screen must stay up carrying the error; there must be no blank editor, and no editor chrome at all. Then repeat with the bundle intact and confirm the ordinary boot is unchanged. **Human, for the reason the whole cluster is.** Both outcomes are drawn into makepad's canvas, so the difference between "start screen with an error" and "blank editor" has no DOM signal and no snapshot entry — `scripts/bundle-boot-check.mjs` can prove the boot chain ran and that the *happy* path logged no failure, which is what it was run for here, but it cannot see which surface is up. And a Windows headless run renders nothing at all (the fork's shader loader is `#[cfg(unix)]`), so the screenshot is blank either way: this one wants Linux, or a human at a browser. |

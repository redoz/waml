---
type: uml.UseCaseDiagram
title: Editor Workflows
description: External roles and shipped workflows at the WAML editor boundary.
profile: uml-domain
---

# Editor Workflows

## Members

### External actors
- [Author](../actors/author.md)
- [Reader](../actors/reader.md)

### WAML editor boundary

#### Author workflows

- [Edit Prose](../workflows/edit-prose.md)
- [Report Every Problem](../workflows/report-every-problem.md)
- [Save and Undo](../workflows/save-and-undo.md)
- [Sequence Language](../workflows/sequence-language.md)

#### Shared workflows

- [Fit the Window](../workflows/fit-the-window.md)
- [Interact with a Class Diagram](../workflows/interact-with-a-class-diagram.md)
- [Select and Inspect](../workflows/select-and-inspect.md)

#### Reader workflows

- [Browse the Tree](../workflows/browse-the-tree.md)
- [Interact with an Activity Diagram](../workflows/interact-with-an-activity-diagram.md)
- [Interact with a Sequence Diagram](../workflows/interact-with-a-sequence-diagram.md)
- [Navigate and Return](../workflows/navigate-and-return.md)
- [Open a Bundle](../workflows/open-a-bundle.md)
- [Read a Document](../workflows/read-a-document.md)
- [Route the Edges](../workflows/route-the-edges.md)
- [Solve the Layout](../workflows/solve-the-layout.md)
- [Use the Shell](../workflows/use-the-shell.md)
- [Work with Tabs](../workflows/work-with-tabs.md)

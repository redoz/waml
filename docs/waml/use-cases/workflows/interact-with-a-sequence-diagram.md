---
type: uml.UseCase
title: Interact with a Sequence Diagram
description: A reader views and interacts with a sequence diagram.
---

# Interact with a Sequence Diagram

## Relationships
- associates [Reader](../actors/reader.md)

## Owning goal

- [Interact with a Sequence Diagram](../../goals/uml/sequence/interact-with-a-sequence-diagram.md)


## Scenarios

- [NATIVE-040](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-040-—-the-sequence-diagram-shows-its-interaction-scene)
- [NATIVE-041](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-041-—-a-message-wins-a-hit-over-its-fragment)
- [NATIVE-042](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-042-—-an-activation-hit-selects-its-lifeline)
- [NATIVE-043](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-043-—-a-fragment-border-is-selectable-but-its-empty-interior-is-not)
- [NATIVE-044](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-044-—-pan-and-zoom-move-the-sequence-camera)
- [NATIVE-054](../../goals/uml/sequence/interact-with-a-sequence-diagram.md#native-054-—-an-unavailable-behavior-document-shows-diagnostic-feedback)

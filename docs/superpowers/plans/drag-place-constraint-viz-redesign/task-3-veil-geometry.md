### Task 3: Veil geometry (pure, testable)

**Files:**
- Create: `crates/waml-editor/src/veil.rs` (pure keep-out geometry + desaturation set + distance fade; unit tests inline)
- Modify: `crates/waml-editor/src/main.rs` (add `mod veil;`)

**Interfaces:**
- Consumes: `waml::solve::Rect` (world rect: `x, y, w, h`), `waml::syntax::Direction`.
- Produces (consumed by Tasks 4/7):
  - `crate::veil::KeepOut { pub x: Option<(f64, f64)>, pub y: Option<(f64, f64)> }` — per locked axis, `(anchor_edge_coord, extend_sign)`; `extend_sign` is `+1.0` or `-1.0`. Cardinal ⇒ one axis `Some`; diagonal ⇒ both.
  - `crate::veil::keep_out(reference: Rect, dir: Direction) -> KeepOut`
  - `crate::veil::in_keep_out(k: &KeepOut, card: Rect) -> bool`
  - `crate::veil::desaturated_cards<'a>(reference: Rect, dir: Direction, cards: &'a [(String, Rect)], subject_key: &str, reference_key: &str) -> Vec<&'a str>`
  - `crate::veil::distance_fade(dist_from_anchor: f64, reach: f64) -> f64` — monotonic non-increasing, `1.0` at the anchor, `0.0` at/after `reach`.

**Direction → anchor mapping (spec §2), world coords (screen-down = +y):**
- `left of` → reference LEFT edge (`x = reference.x`), extends right (`+1`).
- `right of` → reference RIGHT edge (`reference.x + reference.w`), extends left (`-1`).
- `above` → reference TOP edge (`reference.y`), extends down (`+1`).
- `below` → reference BOTTOM edge (`reference.y + reference.h`), extends up (`-1`).
- diagonals = the two matching cardinal half-planes composed on both axes (e.g. `above left of` = top edge extend down + left edge extend right).

This module is 100% pure — no makepad, no `Cx`, no GPU. Mirrors the `node_at` / `handle_rect` unit-test style.

---

- [ ] **Step 1: Create `veil.rs` with the failing tests first**

Create `crates/waml-editor/src/veil.rs`:
...
pub fn keep_out(reference: Rect, dir: Direction) -> KeepOut {
    ...
    match dir {
        LeftOf => KeepOut { x: Some((left, 1.0)), y: None },
        RightOf => KeepOut { x: Some((right, -1.0)), y: None },
        Above => KeepOut { x: None, y: Some((top, 1.0)) },
        Below => KeepOut { x: None, y: Some((bottom, -1.0)) },
        AboveLeft => KeepOut { x: Some((left, 1.0)), y: Some((top, 1.0)) },
        AboveRight => KeepOut { x: Some((right, -1.0)), y: Some((top, 1.0)) },
        BelowLeft => KeepOut { x: Some((left, 1.0)), y: Some((bottom, -1.0)) },
        BelowRight => KeepOut { x: Some((right, -1.0)), y: Some((bottom, -1.0)) },
    }
}
...
mod tests {
    ...
    fn rect(x: f64, y: f64, w: f64, h: f64) -> Rect { Rect { x, y, w, h } }

    #[test]
    fn cardinal_anchors_on_the_correct_edge_and_side() {
        let r = rect(100.0, 50.0, 200.0, 90.0); // left=100 right=300 top=50 bottom=140
        assert_eq!(keep_out(r, Direction::LeftOf), KeepOut { x: Some((100.0, 1.0)), y: None });
        assert_eq!(keep_out(r, Direction::RightOf), KeepOut { x: Some((300.0, -1.0)), y: None });
        assert_eq!(keep_out(r, Direction::Above), KeepOut { x: None, y: Some((50.0, 1.0)) });
        assert_eq!(keep_out(r, Direction::Below), KeepOut { x: None, y: Some((140.0, -1.0)) });
    }

    #[test]
    fn diagonals_lock_both_axes_on_the_matching_corner() {
        let r = rect(100.0, 50.0, 200.0, 90.0);
        assert_eq!(keep_out(r, Direction::AboveLeft), KeepOut { x: Some((100.0, 1.0)), y: Some((50.0, 1.0)) });
        assert_eq!(keep_out(r, Direction::BelowRight), KeepOut { x: Some((300.0, -1.0)), y: Some((140.0, -1.0)) });
    }
    ...
    fn participants_are_exempt_from_desaturation() {
        ...
        assert_eq!(got, vec!["other"], "only the non-participant inside the keep-out desaturates");
    }
    ...
    fn distance_fade_is_monotonic_non_increasing() {
        ...
        let samples: Vec<f64> = [-10.0, 0.0, 50.0, 150.0, 299.0, 300.0, 400.0]
            .iter().map(|&d| distance_fade(d, reach)).collect();
        ...
        for w in samples.windows(2) {
            assert!(w[0] >= w[1], "fade must never increase with distance: {samples:?}");
        }
    }
}
```
...
In `crates/waml-editor/src/main.rs`, add `mod veil;` in alphabetical position (between `mod tree_panel;`... actually between `mod tool_dock;` and `mod tree;` is `veil` out of order; place it after `mod tree_panel;` at the end of the list, or keep alpha: `veil` sorts after `tree_panel`). Add:
...
Run: `cargo test -p waml-editor --lib veil`
Expected: PASS (all six tests). If Step 1 was written before the module compiled, run once to see the module missing, then add it — but since the file ships complete with impl, this goes straight to PASS.
...
```bash
git add crates/waml-editor/src/veil.rs crates/waml-editor/src/main.rs
git commit -m "feat(veil): pure keep-out geometry, desaturation set, distance fade"
```

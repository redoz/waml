# Sequence Flat Model — Task 1: Flatten the sequence runtime model end-to-end

> **Single atomic segment (1 of 1)** of the **Sequence Flat Model** plan — the whole slice lands in ONE green commit: the generated wasm binding (`packages/wasm/src/generated/waml_wasm.d.ts`) couples the Rust runtime shape to its TS consumers, so a Rust-only intermediate would drift the checked-in binding and fail `pnpm build`. Do NOT split this task. See [`README.md`](README.md) for Goal, Architecture, File Structure, Notes/risks, Self-Review, and the corrected cross-slice contract; full rebased plan preserved verbatim as [`_source.md`](_source.md).

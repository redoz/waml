# Task 1: `PanelGeom` — pure centered-panel/scroll geometry

**Files:**
- Create: `crates/waml-editor/src/overlay_shell.rs`
- Modify: `crates/waml-editor/src/main.rs` (add `mod overlay_shell;`)

**Interfaces:**
- Produces (consumed by Task 2 and every page):
  - `pub const PANEL_PAD_V: f64`, `PANEL_PAD_H: f64`, `WINDOW_MARGIN: f64`, `SCROLLBAR_W: f64`, `SCROLLBAR_INSET: f64`, `SCROLLBAR_MIN_THUMB: f64`, `EDGE_H: f64`
  - `pub struct PanelGeom` with `PanelGeom::new(window: DVec2, panel_w: f64, content_h: f64) -> Self`, `panel_height()`, `panel_rect() -> Rect`, `viewport_height()`, `content_width()`, `max_scroll()`, `scroll()`, `set_scroll(f64)`, `content_origin() -> DVec2`, `clip_rect() -> Rect`, `edge_rect() -> Rect`, `thumb_rect() -> Option<Rect>`, `scroll_for_thumb_y(f64) -> f64`. `#[derive(Clone, Copy, Default)]`.

This is the `LinearGeom` idiom (`crates/waml-editor/src/popup/menu.rs`) adapted to a window-centered panel: content clamps to a window-margined max height, scrolls under a clip, and draws a right-inset thumb. Pure — no `Cx`, unit-tested directly.

---

- [ ] **Step 1: Register the module**

In `crates/waml-editor/src/main.rs`, add alphabetically near the other `mod` lines (after `mod node_style;` / before `mod panel_glass;` is fine — order is not load-bearing for a pure module):
...
Create `crates/waml-editor/src/overlay_shell.rs` with ONLY the test module first so the tests fail to compile (drives the API):
...
mod tests {
    ...
    const WIN: DVec2 = DVec2 { x: 1200.0, y: 800.0 };
...
Expected: FAIL — `PanelGeom` / constants not found.

- [ ] **Step 4: Implement `PanelGeom` and the constants**

Insert above the `#[cfg(test)]` module in `overlay_shell.rs`:
...
impl PanelGeom {
    pub fn new(window: DVec2, panel_w: f64, content_h: f64) -> Self {
        Self { window, panel_w, content_h, scroll: 0.0 }
    }
    ...
    pub fn panel_rect(&self) -> Rect {
        ...
        Rect {
            pos: dvec2((self.window.x - self.panel_w) * 0.5, (self.window.y - h) * 0.5),
    ...
    pub fn edge_rect(&self) -> Rect {
        let p = self.panel_rect();
        Rect { pos: p.pos, size: dvec2(self.panel_w, EDGE_H) }
    }
...
```bash
git add crates/waml-editor/src/overlay_shell.rs crates/waml-editor/src/main.rs
git commit -m "feat(overlay): add PanelGeom pure centered-panel scroll geometry"
```

Verification: full gate `cargo test --workspace && pnpm -r test && pnpm lint && pnpm build` green. `PanelGeom` methods are `pub` but unused outside the test module — that is fine here (they are all exercised by the tests, so no dead_code); Task 2 makes them production-reachable.

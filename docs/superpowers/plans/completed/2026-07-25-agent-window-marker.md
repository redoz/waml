...
(`agent_mark.rs`), draws a single mark: a right-floated pill in the caption's title row,
whose fill is the `--color` value and whose text is `--title`. It is mounted
**zero-width** as the first child of `title_row` so it consumes no layout space and
shifts no existing caption widget, then draws with `draw_abs` over a row width the App
measures and pushes in. `App` holds the parsed values and re-pushes them after every
theme reload.

**REVISED 2026-07-25 after visual review — read this before Task 2.** The original design
also washed the whole title row 15% toward the tint. That is **cut**: the title row keeps
its plain white `atlas.field_bg`. The pill is the only mark. The pill must also be
**sized from its own text and centred on the row** — an earlier cut sized it from the row
height, which read too tall and sat too high.
...
Creates the widget, wires its module registration and its DSL mount, and adds the pure
colour helper with tests. It draws nothing yet: nothing has set its marks.
...
Create `crates/waml-editor/src/agent_mark.rs` containing **only** the tests plus the
function signature, so the test failure is a real assertion failure rather than a missing
file:
...
Expected: FAIL — `label_ink_is_near_white_on_a_dark_fill` and
`label_ink_weights_green_over_blue` fail on the stub's constant return.
...
```rust
//! Per-agent window marker: a right-floated pill in the caption's title row,
//! driven by the `--title` / `--color` launch flags so several
//! concurrently-running editor windows can be told apart by eye.
//!
//! The pill is the ONLY mark. An earlier cut also washed the whole title row
//! 15% toward the tint; that read as a broken theme rather than a marker and was
//! dropped on review, so the row keeps its plain `atlas.field_bg`.
//!
...
//! the model name keep their exact positions). Everything is drawn with
//! `draw_abs` across a row width
...
script_mod! {
    ...
    mod.widgets.AgentMark = set_type_default() do mod.widgets.AgentMarkBase{
        ...
        // reload and the mark follows the palette for free.
...
pub struct AgentMark {
    ...
    #[rust]
    badge: Option<String>,
    /// `--color`. `None` falls back to `chip_fallback`.
    #[rust]
    tint: Option<Vec4>,
...
```
                            ...
                            // not move. It right-floats its pill via `draw_abs`
                            // and an App-measured row width (`sync_agent_row`),
                            // bounded by this row's `clip_x`.
...
Expected: the caption looks **exactly** as it did before this task — no pill, white title
row, burger and model name unmoved. The widget is mounted but has nothing to draw.
...
```
Expected: a white title row with a pink `agent-a` pill at its right end, the pill hugging
the word and vertically centred in the row.
...
param(
    ...
    [switch]$Empty,
    [switch]$Optimized,
    # Per-agent window marker: pill text and pill colour, so several
...
- [ ] The pill sits at the right end of the title row, clear of min/max/close.
- [ ] **The title row's background is still plain white `field_bg`** — the pill is the
      only tinted pixel. Any band of colour across the row is the cut wash and is a defect.
- [ ] **The pill is vertically centred in the row** — equal gap above and below. Measure
      it on the screenshot rather than eyeballing; a top-anchored pill was the exact
      defect this revision fixes.
- [ ] **The pill hugs its word**: roughly `CHIP_PAD_Y` of fill above and below the glyphs,
      not a block spanning most of the 34px row.
...
- [ ] Dragging the caption still moves the window, and dragging **on the pill** does too
      (the mark must not have become a drag dead-zone).
- [ ] A long model path still clips at the row edge without colliding with the pill.
- [ ] Pressing `T` toggles the theme and **the mark survives**, its fallback colours
      re-sourced from the new palette.
- [ ] `--title` alone renders a legible Atlas-coloured pill.
- [ ] `--color` alone renders a bare tinted swatch pill, no text.
- [ ] A no-dir launch (`-Empty -Color '#2b8'`) shows the swatch on the start screen.
...
| Colour maths (ink pick) | Task 2 unit tests (`cargo test -p waml-editor --lib agent_mark::`) |

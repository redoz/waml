...
- **Do NOT touch `FlowDoc`/`FlowNode`/`FlowEdge`** — the behavior/flow slice is a separate parallel plan.
...
- `crates/waml/src/model.rs` — replace `Lifeline` (508-525), `SeqOperand` (527-539), `SeqItem` (541-562), and reshape `SequenceDoc` (564-579) with `SeqChild`, `SeqEdge`, `SeqNode`, `SequenceDoc`. `MessageVerb` (441-475) and `FragmentKind` (477-505) are unchanged.
- `crates/waml/src/parse.rs` — import list (12-15); rewrite `build_interactions` (778-849); rewrite the `builds_sequence_doc_with_resolved_lifelines_and_nested_messages` test (1427-1453).
...
- [ ] **Step 1.3: Replace the sequence model types in `model.rs`.** In `crates/waml/src/model.rs`, replace the four items `Lifeline` (508-525), `SeqOperand` (527-539), `SeqItem` (541-562), and `SequenceDoc` (564-579) — i.e. the whole span from the `/// A sequence participant…` doc comment down to the end of the `SequenceDoc` struct — with:
...
- [ ] **Step 1.4: Update the `parse.rs` import list.** In `crates/waml/src/parse.rs`, change the `use crate::model::{…}` block (lines 12-15) — replace the sequence type names `Lifeline`, `SeqItem`, `SeqOperand` with `SeqChild`, `SeqEdge`, `SeqNode` (keep `SequenceDoc`; leave every other name untouched):

```rust
use crate::model::{
    Attribute, BehaviorKind, Diagram, DiagramDisplay, DiagramGroup, Edge, ElementType, FlowDoc,
    FlowEdge, FlowFlavor, FlowNode, Model, Node, SeqChild, SeqEdge, SeqNode, SequenceDoc,
};
```
...
- [ ] **Step 1.10: Update the `@waml/wasm` re-export barrel.** In `packages/wasm/src/index.ts`, in the `export type { … } from "./generated/waml_wasm.js";` block, replace the three lines `Lifeline,` / `SeqItem,` / `SeqOperand,` (currently lines 47-49) with the new names, keeping `SequenceDoc` and the surrounding entries:
...
- [ ] **Step 1.11: Update the `@waml/okf` re-export barrel.** In `packages/okf/src/types.ts`, in the `export type { … } from "@waml/wasm";` block, replace the three lines `Lifeline,` / `SeqOperand,` / `SeqItem,` (currently lines 20-22) so the sequence names read:
...
## Cross-slice contract (for the parallel behavior/flow plan)

- **No element promotion this round.** Both slices keep behavior docs as self-contained views on `Model.flows` / `Model.interactions`; neither moves interaction/activity elements or messages into the shared `Model.nodes` / `Model.edges` pool (that is spec §4 / §7.2 proper, a later slice). Messages explicitly stay interaction-local (§6). The flow plan must NOT relocate `FlowNode`/`FlowEdge` into the shared pool either, to keep the two slices symmetric.
- **Shared "unified shape" vocabulary:** behavior docs expose `nodes` + `edges`. `FlowDoc` already does; `SequenceDoc` now does too, plus an `items` ordered-containment stream (sequences add order + nesting that flows lack). Type names are substrate-specific and non-colliding: `FlowNode`/`FlowEdge` (flow) vs `SeqNode`/`SeqEdge`/`SeqChild` (sequence). The flow plan should keep its `FlowNode`/`FlowEdge` names; do not introduce a shared `Node`/`Edge` behavior type that would collide with the structural `Model.nodes`/`Model.edges`.
- **`ElementType`/`is_classifier()` (slice 1) is the shared classifier predicate** both slices rely on unchanged; neither widens `BehaviorKind` or the metaclass set here.

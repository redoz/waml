...
- **`Section` gains error nodes.** A malformed line inside a bullet section is preserved as an explicit error node rather than dropped, so `parse` is no longer lossy for the cases `validate` currently re-derives.

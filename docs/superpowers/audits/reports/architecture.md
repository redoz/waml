...
- `crates/waml-editor/src/editor_session.rs:755` (`promote_source_edit`) later validates the session document revision and accepted source `Arc` identity before it installs a source-only revision. `install_semantic_completion` starts at line 887 and rejects stale identity or revision results.
...
- `crates/waml-editor/src/app/actions.rs:983` calls `run_semantic_request`, line 985 calls `install_semantic_completion`, and the promotion-failure branch calls `DocumentHost::sync_active` at line 1005 so the active Markdown view returns to the unchanged editor-session snapshot.

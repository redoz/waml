...
> **Provenance.** This copy is the whole spec, replaced wholesale at v0.2
> (`ddcc0dc0`, 2026-08-05) over the v0.1 added in `d82c351e` (2026-07-12).
> Both commits are by this repository's owner and neither records an upstream
> revision, so the repo cannot currently answer whether OKF is maintained
> elsewhere. Two things follow, and they need the owner's answer:
>
> - **If OKF is external**, the missing upstream URL and revision are the
>   defect: a wholesale replace cannot be reviewed as a diff, and drift is
>   undetectable. Record the URL and commit here, and add a CI check that
>   diffs this file against that revision.
> - **If OKF is this project's own spec**, there is nothing to track and this
>   note should say so — and the WAML profile in
>   [waml-okf-extensions.md](./waml-okf-extensions.md) is a first-party
>   extension rather than a fork awaiting upstreaming.
>
> Do not delete this note without answering the question; "no upstream
> recorded" reads identically to "no upstream exists", which is why the
> ambiguity survived two versions.
